...
static int MeasureString ( const std::string &s )
{
...
//
static int DrawCharacter ( int left, int top, char c, Color color )
{
	unsigned int glyph = Font [ c ];
	int width = glyph >> 28;
...
static void DrawString ( int x, int y, const std::string &s, const Color color, bool shadow = false )
{
	for ( char c : s )
	{
		// The +1 is for the space between letters
		if ( shadow )
			DrawCharacter ( x, y + 1, c, Black );
